...
@Component({
  ...
  templateUrl: './global-role-form.component.html',
  styleUrls: ['./global-role-form.component.scss']
})
export class GlobalRoleFormComponent implements OnInit {
  roleForm!: FormGroup;
  permissionGroups: MenuPermissionGroup[] = [];
  baseRoles: GlobalRole[] = [];
  isLoadingBaseRoles = false;
  ...
  ngOnInit(): void {
    const state = history.state as { mode?: 'add' | 'edit' | 'view'; role?: GlobalRole };
  ...
  private loadData(): void {
    ...
    this.isLoadingBaseRoles = true;

    forkJoin({
      menus: this.superAdminService.getMenus(),
      roles: this.superAdminService.getGlobalRoles()
    }).subscribe({
      next: ({ menus, roles }) => {
        this.buildPermissionGroups(menus.data);
        this.baseRoles = roles;
  ...
  onBaseRoleChange(selectedRoleId: string): void {
    ...
    const selectedRole = this.baseRoles.find(r => r.roleId === selectedRoleId);
    if (selectedRole?.menus) {
      this.resetPermissions();
      this.populateExistingPermissions(selectedRole.menus);
    }
  }
  ...
  getMenuIcon(menuName: string): string {
    const iconMap: Record<string, string> = {
      ...
      'Billings': 'account_balance',
      'Employee Dashboard': 'space_dashboard',
      'Admin Dashboard': 'admin_panel_settings',
      'Holidays': 'celebration',
      'Help Desk': 'support_agent',
      'Payroll': 'payments'
    };
  ...
  onBack(): void {
    this.router.navigate(['/platform-admin/global-roles']);
  }
  ...
  onSubmit(): void {
    ...
    const payload: CreateGlobalRoleRequest = {
    ...
    const request$ = this.isEditMode
      ? this.superAdminService.updateGlobalRole(this.role!.roleId, payload)
      : this.superAdminService.createGlobalRole(payload);

    request$.subscribe({
      next: () => {
        this.isSaving = false;
        this.notificationService.showSuccess(
          this.isEditMode ? 'Global role updated successfully' : 'Global role created successfully'
        );
        this.onBack();
      },
      error: (err) => {
        ...
        const message = err?.message
          || (this.isEditMode ? 'Failed to update global role' : 'Failed to create global role');
